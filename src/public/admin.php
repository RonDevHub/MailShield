        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            if ($data) {
                try {
                    ...
                    $db->commit();
                    $success = $lang['database_migrated'];
                } catch (Exception $e) {
                    $db->rollBack();
                    $error = $lang['database_migrated_error'] . $e->getMessage();
                }
            }
        }
    }
...
<head> 
    <meta charset="UTF-8">
    <title><?= $lang['admin_title'] ?> | <?= $lang['title'] ?></title>
    ...
    <div class="max-w-md w-full">
        <?php if (!isset($_SESSION['admin_logged_in'])): ?>
            <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700">
                ...
                <form method="POST" class="space-y-4">
                    <input type="text" name="user" placeholder="<?= $lang['placeholder_username'] ?>" required class="w-full p-4 bg-gray-100 dark:bg-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500">
                    <input type="password" name="pass" placeholder="<?= $lang['placeholder_password'] ?>" required class="w-full p-4 bg-gray-100 dark:bg-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500">
                    <button name="login" class="w-full bg-blue-600 py-4 rounded-xl font-bold hover:bg-blue-700 transition-all text-white"><?= $lang['login'] ?></button>
        ...
        <?php else: ?>
            <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 text-center">
                <div class="mb-6">
                    <h1 class="text-2xl font-bold"><?= $lang['admin_dashboard'] ?></h1>
                    <span class="text-[10px] uppercase tracking-widest opacity-30"><?= $lang['admin_database'] ?> <?= strtoupper($driver) ?></span>
                </div>
                ...
                <div class="space-y-4">
                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700 space-y-3">
                        <p class="text-xs font-bold opacity-50 uppercase tracking-tighter"><?= $lang['json_backup'] ?></p>
                        <form method="POST">
                    ...
                    <form method="POST">
                        <button name="export_db" class="w-full border border-gray-200 dark:border-gray-700 py-3 rounded-xl text-xs opacity-50 hover:opacity-100 transition-all"><?= $lang['sqlite_backup'] ?></button>
                    </form>
                    <?php endif; ?>
                    ...
                    <a href="?logout" class="block w-full bg-gray-200 dark:bg-gray-700 py-4 rounded-xl font-bold italic opacity-50 hover:opacity-100 transition-all"><?= $lang['admin_logout'] ?></a>
                </div>
            </div>
